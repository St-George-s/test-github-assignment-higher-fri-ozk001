...
def unreturned(fee, returned, dateRented):
    for x in range(120):
...
def gen_fee(tool_name, dateRented, fee):
    with open('lateTools.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        for x in range(len(tool_name)):
            if fee[x] != 0:
                writer.writerow([tool_name[x], dateRented[x], fee[x]])
    pass

#main program
tool_name, manufacturer, dateRented, returned, fee = get_tool_data()
#count_occurrences(manufacturer)
unreturned(fee, returned, dateRented)
